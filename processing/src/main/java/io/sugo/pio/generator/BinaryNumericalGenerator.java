...
public abstract class BinaryNumericalGenerator extends FeatureGenerator {
    ...
    private static final Logger log = new Logger(BinaryNumericalGenerator.class);
    ...
    public void generate(DataRow data) throws GenerationException {
        try {
            ...
            if (Double.isInfinite(r)) {
                ...
                // LogService.WARNING);
                log.warn("io.sugo.pio.generator.BinaryNumericalGenerator.infinite_value_generated", getFunction());
            }
            if (Double.isNaN(r)) {
                // LogService.getGlobal().log(getFunction() + ": NaN generated.",
                // LogService.WARNING);
                log.warn("io.sugo.pio.generator.BinaryNumericalGenerator.nan_generated",
                        getFunction());
            }
