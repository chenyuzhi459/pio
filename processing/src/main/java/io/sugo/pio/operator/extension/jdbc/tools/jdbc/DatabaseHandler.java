...
import io.sugo.pio.operator.extension.jdbc.tools.jdbc.connection.DatabaseConnectionService;
import io.sugo.pio.operator.extension.jdbc.tools.jdbc.connection.FieldConnectionEntry;
...
//            LogService.getRoot().log(Level.CONFIG, "io.sugo.pio.tools.jdbc.DatabaseHandler.connecting_to_database", new Object[]{this.databaseURL, this.user});
            int loginTimeout = 60;

            try {
                loginTimeout = Integer.parseInt(ParameterService.getParameterValue("rapidminer.tools.db.login.timeout"));
    ...
    public void createTable(ExampleSet exampleSet, TableName tableName,
                            int overwriteMode, boolean firstAttempt,
                            int defaultVarcharLength,
                            boolean addAutoGeneratedPrimaryKeys,
                            String generatedPrimaryKeyAttributeName,
                            int batchSize, Operator operator
    ) throws SQLException, ProcessStoppedException, IllegalArgumentException {
        Statement statement = this.createStatement(false);
            ...
            try {
                if (batchSizeInsertStatement != null) {
                    ...
                    if (needToCommitBatch) {
                        ...
                        needToCommitBatch = false;
                        if (addAutoGeneratedPrimaryKeys) {
                            ResultSet var105 = batchSizeInsertStatement.getGeneratedKeys();
                            Throwable var107 = null;

                            try {
                                while (var105.next()) {
                                    int var108 = var105.getInt(1);
                                    generatedKeysList.add(Integer.valueOf(var108));
                                }
                    ...
                    if (addAutoGeneratedPrimaryKeys) {
                        ...
                        for (int var106 = 0; var106 < generatedKeysList.size(); ++var106) {
                            exampleSet.getExample(var106).setValue(var104, (double) ((Integer) generatedKeysList.get(var106)).intValue());
                        }
                    }
    ...
    public Map<TableName, List<ColumnIdentifier>> getAllTableMetaData(ProgressListener progressListener, int minProgress, int maxProgress, boolean fetchColumns) throws SQLException {
        if (this.connection == null) {
            ...
            int var28 = tableNameList.size();
            int var29 = 0;

            while (var27.hasNext()) {
                TableName var30 = (TableName) var27.next();
                if (progressListener != null && var28 > 0) {
                    progressListener.setCompleted(var29 * (maxProgress - minProgress) / var28 + minProgress);
                }
...
//                        LogService.getRoot().log(Level.WARNING, I18N.getMessage(LogService.getRoot().getResourceBundle(), "com.rapidminer.tools.jdbc.DatabaseHandler.fetching_column_metadata_error", new Object[]{var30, var23}), var23);
    ...
    public static ConnectionEntry getConnectionEntry(ParameterHandler parameterHandler, String repositoryName) {
        try {
            int e = parameterHandler.getParameterAsInt("define_connection");
            switch (e) {
                case 0:
...
//        types.add(type5);
    ...
    public void updateTable(Operator operator, ExampleSet exampleSet, TableName selectedTableName, Set<Attribute> idAttributes, Logger logger) throws SQLException, OperatorException {
        if (exampleSet.getAttributes().size() == 0 && exampleSet.getAttributes().getId() == null) {
            ...
            Attribute var116;
            String var118;
            for (; it.hasNext(); allAttList.add(var116)) {
                var116 = (Attribute) it.next();
                var118 = var116.getName();
                if (!columnTableNames.contains(var118)) {
                    throw new SQLException("Table does not contain column for attribute " + var118 + "!");
                }

                if (!idAttNameList.contains(var118)) {
                    ++attCount;
                    attList.add(var116);
                    var114.append(sc.makeIdentifier(var118) + " = ?");
                    var114.append(", ");
                    insertAttStringBuilder.append(sc.makeIdentifier(var118) + ", ");
                    insertValStringBuilder.append("?, ");
                } else {
                    insertAttStringBuilder.append(sc.makeIdentifier(var118) + ", ");
                    insertValStringBuilder.append("?, ");
                    var115.append(sc.makeIdentifier(var118) + " = ?");
                    var115.append(" AND ");
                }
            }
            ...
            var118 = "INSERT INTO " + sc.makeIdentifier(selectedTableName) + " " + insertAttStringBuilder.toString() + " VALUES " + insertValStringBuilder.toString();
            ...
            try {
                PreparedStatement prepInsertStatement = this.createPreparedStatement(var118, false);
                    ...
                    try {
                        ...
                        while (var28.hasNext()) {
                            ...
                            if (operator != null) {
                                ++counter;
                                if (counter % 50 == 0) {
                                    operator.getProgress().setCompleted(counter);
                                }
                            }
                            ...
                            while (updatedRowCount.hasNext()) {
                                ...
                                if (rs.isNumerical()) {
                                    ...
                                    Timestamp var122 = new Timestamp((new Double(ex.getValue(rs))).longValue());
                                    ++idCount;
                                    prepUpdateStatement.setTimestamp(attCount + idCount, var122);
                                    prepInsertStatement.setTimestamp(allAttList.indexOf(rs) + 1, var122);
                                    prepSelectStatement.setTimestamp(idAttNameList.indexOf(rs.getName()) + 1, var122);
                                }
                            }
                            ...
                            while (updatedRowCount.hasNext()) {
                                rs = (Attribute) updatedRowCount.next();
                                double var123 = ex.getValue(rs);
                                if (Double.isNaN(var123)) {
                                    ...
                                    Date var125 = new Date((new Double(ex.getValue(rs))).longValue());
                                    prepUpdateStatement.setDate(attList.indexOf(rs) + 1, var125);
                                    prepInsertStatement.setDate(allAttList.indexOf(rs) + 1, var125);
                                } else if (rs.getValueType() == 11) {
                                    Time var126 = new Time((new Double(ex.getValue(rs))).longValue());
                                    prepUpdateStatement.setTime(attList.indexOf(rs) + 1, var126);
                                    prepInsertStatement.setTime(allAttList.indexOf(rs) + 1, var126);
                                } else if (rs.getValueType() == 9) {
                                    Timestamp var127 = new Timestamp((new Double(ex.getValue(rs))).longValue());
                                    prepUpdateStatement.setTimestamp(attList.indexOf(rs) + 1, var127);
                                    prepInsertStatement.setTimestamp(allAttList.indexOf(rs) + 1, var127);
                                }
                            }
                            ...
                            if (attCount > 0) {
                                ...
                                ResultSet var120 = this.executeQuery(prepSelectStatement, operator);
                                Throwable var124 = null;

                                try {
                                    if (var120.next()) {
                                        var119 = var120.getInt(1);
                                    }
...
//                    LogService.getRoot().log(Level.WARNING, "io.sugo.pio.tools.jdbc.DatabaseHandler.cancel_request_error");
                } finally {
