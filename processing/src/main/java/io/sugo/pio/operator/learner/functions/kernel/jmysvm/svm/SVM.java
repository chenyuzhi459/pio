...
import io.sugo.pio.operator.learner.functions.kernel.AbstractMySVMLearner;
...
import io.sugo.pio.operator.learner.functions.kernel.jmysvm.optimizer.QuadraticProblem;
import io.sugo.pio.operator.learner.functions.kernel.jmysvm.optimizer.QuadraticProblemSMO;
import io.sugo.pio.operator.learner.functions.kernel.jmysvm.util.MaxHeap;
import io.sugo.pio.operator.learner.functions.kernel.jmysvm.util.MinHeap;
...
public abstract class SVM implements SVMInterface {
    ...
    private static final Logger logger = new Logger(SVM.class);
    ...
    public SVM(Operator paramOperator, Kernel new_kernel, SVMExamples new_examples,
               io.sugo.pio.example.ExampleSet rapidMinerExamples, RandomGenerator randomGenerator)
        ...
        if (weightAttribute != null) {
            Iterator<io.sugo.pio.example.Example> reader = rapidMinerExamples.iterator();
            int index = 0;
            while (reader.hasNext()) {
                io.sugo.pio.example.Example example = reader.next();
    ...
    protected boolean convergence() {
        ...
        if (Math.abs(alpha_sum + sum_alpha) > convergence_epsilon) {
    ...
    protected double lambda(int i) {
        ...
        double result;
        if (is_alpha_neg(i)) {
            result = -Math.abs(nabla(i) + lambda_eq);
        } else {
            result = -Math.abs(nabla(i) - lambda_eq);
        }
        ;
