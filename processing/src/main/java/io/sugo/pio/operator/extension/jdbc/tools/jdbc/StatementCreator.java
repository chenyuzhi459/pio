...
//        LogService.getRoot().log(Level.FINE, "io.sugo.pio.tools.jdbc.StatementCreator.initialization_of_quote_character", this.identifierQuote);
        Map dataTypeToMDMap = null;
        ResultSet typesResult = null;
        if(dbMetaData.getURL() != null && CACHE.containsKey(dbMetaData.getURL())) {
            dataTypeToMDMap = (Map)CACHE.get(dbMetaData.getURL());
        } else {
            if(dbMetaData.getDatabaseProductName().contains("PostgreSQL")) {
                dataTypeToMDMap = PostgreSQLUtils.POSTGRES_DATA_TYPES;
            } else {
                dataTypeToMDMap = new HashMap();
                ...
                try {
                    ...
                    label88:
                    while(true) {
                        DataTypeSyntaxInformation dtmd;
                        do {
                            if(!typesResult.next()) {
                                break label88;
                            }

                            dtmd = new DataTypeSyntaxInformation(typesResult, dbMetaData.getDatabaseProductName());
                        } while(((Map)dataTypeToMDMap).containsKey(Integer.valueOf(dtmd.getDataType())) && !this.isUCanAccessVarchar(dbMetaData, (Map)dataTypeToMDMap, dtmd));

                        ((Map)dataTypeToMDMap).put(Integer.valueOf(dtmd.getDataType()), dtmd);
                    }
                } finally {
                    if(typesResult != null) {
                        typesResult.close();
            ...
            if(dbMetaData.getURL() != null) {
                CACHE.put(dbMetaData.getURL(), dataTypeToMDMap);
            }
        }

        this.registerSyntaxInfo(1, (Map)dataTypeToMDMap, new int[]{12});
        this.registerSyntaxInfo(5, (Map)dataTypeToMDMap, new int[]{2005, 2004, -1, -16, 12});
        this.registerSyntaxInfo(4, (Map)dataTypeToMDMap, new int[]{8, 7, 6});
        this.registerSyntaxInfo(2, (Map)dataTypeToMDMap, new int[]{8, 7, 6});
        this.registerSyntaxInfo(3, (Map)dataTypeToMDMap, new int[]{4});
        this.registerSyntaxInfo(10, (Map)dataTypeToMDMap, new int[]{91, 93});
        this.registerSyntaxInfo(9, (Map)dataTypeToMDMap, new int[]{93});
        this.registerSyntaxInfo(11, (Map)dataTypeToMDMap, new int[]{92, 93});
        this.registerSyntaxInfo(0, (Map)dataTypeToMDMap, new int[]{8, 7, 6});
        this.registerSyntaxInfo(6, (Map)dataTypeToMDMap, new int[]{12});
    }
...
//                LogService.getRoot().log(Level.FINE, "io.sugo.pio.tools.jdbc.StatementCreator.mapping_ontology_value_type", new Object[]{Ontology.ATTRIBUTE_VALUE_TYPE.mapIndex(attributeType), si});
                return;
            }
        }

//        LogService.getRoot().log(Level.WARNING, "io.sugo.pio.tools.jdbc.StatementCreator.no_sql_value_type_found", Ontology.ATTRIBUTE_VALUE_TYPE.mapIndex(attributeType));
    }
