...
import io.sugo.pio.parameter.MetaDataProvider;
...
//	public static final String PARAMETER_ATTRIBUTES_SEPERATOR = "\\|";
    ...
    @Override
    public List<ParameterType> getParameterTypes(ParameterHandler operator, final Port port, int... valueTypes) {
        ...
        ParameterType type = new ParameterTypeAttributes(PARAMETER_ATTRIBUTES, I18N.getMessage("pio.AttributeFilter.subset.attributes"),
                port, false, valueTypes);
        types.add(type);
        return types;
    }

    public List<ParameterType> getParameterTypes(String key, String description, ParameterHandler operator, final Port port, int... valueTypes) {
        List<ParameterType> types = new LinkedList<ParameterType>();
        ParameterType type = new ParameterTypeAttributes(key, description,
                port, false, valueTypes);
        types.add(type);
        return types;
    }
