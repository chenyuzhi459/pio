...
public class SVMExamples implements Serializable {
    ...
    private static Map<Integer, MeanVariance> createMeanVariances(io.sugo.pio.example.ExampleSet exampleSet) {
        double[] sum = new double[exampleSet.getAttributes().size()];
        double[] squaredSum = new double[sum.length];

        Iterator<io.sugo.pio.example.Example> reader = exampleSet.iterator();
    ...
    public SVMExamples(io.sugo.pio.example.ExampleSet exampleSet, Attribute labelAttribute, boolean scale) {
     ...
     */
    public SVMExamples(io.sugo.pio.example.ExampleSet exampleSet, Attribute labelAttribute,
        ...
        while (reader.hasNext()) {
            io.sugo.pio.example.Example current = reader.next();
            Map<Integer, Double> attributeMap = new LinkedHashMap<Integer, Double>();
            int a = 0;
            for (Attribute attribute : exampleSet.getAttributes()) {
                double value = current.getValue(attribute);
                if (!io.sugo.pio.example.Tools.isDefault(attribute.getDefault(), value)) {
