...
public class ExampleResolver implements Resolver {
    ...
    public static final String KEY_ATTRIBUTES = "regular attributes";//I18N.getGUIMessage("gui.dialog.function_input.regular_attributes");
    public static final String KEY_SPECIAL_ATTRIBUTES = "special attributes";//I18N.getGUIMessage("gui.dialog.function_input.special_attributes");

    /**
     * Creates an {@link ExampleResolver} that can bind {@link Example}s that have the same meta
     * data as the given exampleSet.
     *
     * @param exampleSet the {@link ExampleSet} from which this resolver should bind examples, cannot be
     *                   {@code null}
     */
    ...
    public Collection<FunctionInput> getAllVariables() {
        ...
        for (AttributeMetaData amd : metaDataAttributes) {
            if (amd.isSpecial()) {
                functionInputs.add(new FunctionInput(FunctionInput.Category.DYNAMIC, KEY_SPECIAL_ATTRIBUTES, amd.getName(),
                        amd.getValueType(), amd.getRole()));
            } else {
                functionInputs
                        .add(new FunctionInput(FunctionInput.Category.DYNAMIC, KEY_ATTRIBUTES, amd.getName(), amd.getValueType(), null));
            }
        }
