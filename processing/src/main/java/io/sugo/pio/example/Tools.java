...
import io.sugo.pio.example.set.AttributeWeightedExampleSet;
...
import io.sugo.pio.generator.FeatureGenerator;
import io.sugo.pio.operator.Operator;
import io.sugo.pio.operator.OperatorCreationException;
import io.sugo.pio.operator.OperatorException;
import io.sugo.pio.operator.UserError;
import io.sugo.pio.operator.preprocessing.IdTagging;
import io.sugo.pio.operator.tools.math.sampling.OrderedSamplingWithoutReplacement;
...
public class Tools {
	...
	private static final Logger log = new Logger(Tools.class);
	...
	public static Attribute createWeightAttribute(ExampleSet exampleSet) {
		...
		if (weight != null) {
			log.warn("ExampleSet.createWeightAttribute(): Overwriting old weight attribute!");
		}
	...
	public static void replaceValue(ExampleSet exampleSet, Attribute attribute, double oldValue, double newValue) {
		for (Example example : exampleSet) {
			...
			if (io.sugo.pio.tools.Tools.isEqual(value, oldValue)) {
				example.setValue(attribute, newValue);
			}
		}
	}
