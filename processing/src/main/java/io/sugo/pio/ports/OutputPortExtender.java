...
import io.sugo.pio.ports.metadata.OneToManyPassThroughRule;
...
public class OutputPortExtender extends SinglePortExtender<OutputPort> {
	...
	public OutputPortExtender(String name, String description, Ports<OutputPort> ports) {
	...
	public void deliver(List<? extends IOObject> inputs) {
		...
		for (OutputPort port : getManagedPorts()) {
			if (port.isConnected()) {
				if (i >= inputs.size()) {
					getPorts().getOwner().getOperator().getLogger().info("Insufficient input for " + port.getSpec());
				} else {
					...
					if (input != null) {
						...
						getPorts().getOwner().getOperator().getLogger().info("Delivering " + name + " to " + port.getSpec());
					}
				}
				i++;
			}
