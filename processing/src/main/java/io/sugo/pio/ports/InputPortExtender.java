...
public class InputPortExtender extends SinglePortExtender<InputPort> {
	...
	public InputPortExtender(String name, String description, Ports<InputPort> ports) {
		this(name, description, ports, null, 0);
	}

	public InputPortExtender(String name, String description, Ports<InputPort> ports, MetaData desiredMetaData, boolean firstIsMandatory) {
		this(name, description, ports, desiredMetaData, firstIsMandatory ? 1 : 0);
	}

	public InputPortExtender(String name, String description, Ports<InputPort> ports, MetaData desiredMetaData, int numberOfMandatory) {
		super(name, description, ports);
			...
			if (data != null) {
				if (unfold && (data instanceof IOObjectCollection)) {
					...
					if (desiredClass.isInstance(data)) {
						...
						throw new UserError(getPorts().getOwner().getOperator(), "pio.error.operator.exampleset_wrong_input_type", data
								.getClass().getSimpleName(), port.getName(), desiredClass.getSimpleName());
					}
				}
			}
		}
		...
		for (IOObject obj : collection.getObjects()) {
			if (obj instanceof IOObjectCollection) {
				...
				if (desiredClass.isInstance(obj)) {
					...
					throw new UserError(getPorts().getOwner().getOperator(), "pio.error.operator.exampleset_wrong_input_type", obj.getClass().getSimpleName(),
							port.getName(), desiredClass.getSimpleName());
				}
			}
		}
	}
