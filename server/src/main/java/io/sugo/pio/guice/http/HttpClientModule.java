...
package io.sugo.pio.guice.http;
...
public class HttpClientModule implements Module
{
  ...
  {
    if (annotation != null) {
      JsonConfigProvider.bind(binder, propertyPrefix, PioHttpClientConfig.class, annotation);
      binder.bind(HttpClient.class)
            .annotatedWith(annotation)
            .toProvider(new HttpClientProvider(annotation))
            .in(LazySingleton.class);
    } else if (annotationClazz != null) {
      JsonConfigProvider.bind(binder, propertyPrefix, PioHttpClientConfig.class, annotationClazz);
      binder.bind(HttpClient.class)
            .annotatedWith(annotationClazz)
            .toProvider(new HttpClientProvider(annotationClazz))
            .in(LazySingleton.class);
    } else {
      JsonConfigProvider.bind(binder, propertyPrefix, PioHttpClientConfig.class);
      binder.bind(HttpClient.class)
  ...
  public static class HttpClientProvider extends AbstractHttpClientProvider<HttpClient>
  {
    ...
    {
      final PioHttpClientConfig config = getConfigProvider().get().get();
