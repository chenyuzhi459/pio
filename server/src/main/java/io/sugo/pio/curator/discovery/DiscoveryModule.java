...
import io.sugo.pio.server.initialization.CuratorDiscoveryConfig;
...
/**
 * The DiscoveryModule allows for the registration of Keys of PioNode objects, which it intends to be
...
public class DiscoveryModule implements Module
{
  ...
  {
    registerKey(binder, Key.get(new TypeLiteral<PioNode>(){}));
  }
  ...
  {
    registerKey(binder, Key.get(new TypeLiteral<PioNode>(){}, annotation));
  }
   ...
   *
   * @param binder the Binder to register with
   * @param annotation The annotation class to use in finding the PioNode instance
   */
   ...
   *
   * @param binder the Binder to register with
   * @param key The key to use in finding the PioNode instance
   */
  public static void registerKey(Binder binder, Key<PioNode> key)
  {
    PioBinders.discoveryAnnouncementBinder(binder).addBinding().toInstance(new KeyHolder<>(key));
  ...
  public void configure(Binder binder)
  {
    ...
    PioBinders.discoveryAnnouncementBinder(binder);
  ...
  public CuratorServiceAnnouncer getServiceAnnouncer(
      ...
      final Set<KeyHolder<PioNode>> nodesToAnnounce,
  ...
  ) throws Exception
  {
    lifecycle.addMaybeStartHandler(
        ...
        {
          private volatile List<PioNode> nodes = null;
          ...
          public void start() throws Exception
          {
            if (nodes == null) {
              ...
              for (KeyHolder<PioNode> holder : nodesToAnnounce) {
                nodes.add(injector.getInstance(holder.getKey()));
              }
            }

            for (PioNode node : nodes) {
