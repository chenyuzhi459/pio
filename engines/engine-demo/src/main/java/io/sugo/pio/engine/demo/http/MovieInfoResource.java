...
public class MovieInfoResource {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getInfoByMovIds(
            InputStream in,
            @Context final HttpServletRequest req
    ) {
        try {
            ObjectMapper jsonMapper = ObjectMapperUtil.getObjectMapper();
            List<String> movieIds = jsonMapper.readValue(in, new TypeReference<List<String>>() {});
            List<Map<String, Object>> movieInfos = new ArrayList<>(movieIds.size());
            ...
            String str = jsonMapper.writeValueAsString(movieInfos);
            return Response.status(Response.Status.ACCEPTED).entity(str).build();
        } catch (IOException e) {
            e.printStackTrace();
        } catch (Exception e) {
